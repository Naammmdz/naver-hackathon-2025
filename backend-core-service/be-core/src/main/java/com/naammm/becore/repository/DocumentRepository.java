...
public interface DocumentRepository extends JpaRepository<Document, String> {
    ...
    List<Document> findByWorkspaceIdAndTrashedFalseOrderByUpdatedAtDesc(String workspaceId);
    
    List<Document> findByWorkspaceIdAndParentIdAndTrashedFalseOrderByUpdatedAtDesc(String workspaceId, String parentId);
    
    List<Document> findByWorkspaceIdAndTrashedTrueOrderByUpdatedAtDesc(String workspaceId);
    
    @Query("SELECT d FROM Document d WHERE d.workspaceId = :workspaceId AND d.trashed = false AND " +
           "(LOWER(d.title) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(d.content) LIKE LOWER(CONCAT('%', :search, '%')))")
    List<Document> searchDocumentsByWorkspace(@Param("workspaceId") String workspaceId, @Param("search") String search);
}
