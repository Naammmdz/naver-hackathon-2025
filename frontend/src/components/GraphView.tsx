...
export function GraphView({ workspaceId, onNodeClick }: GraphViewProps) {
  ...
  return (
    <div className="relative w-full h-full">
      {/* Loading overlay */}
      {loading && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-gradient-to-b from-slate-950 to-slate-900">
          <div className="flex flex-col items-center gap-3">
            <Loader2 className="h-8 w-8 animate-spin text-blue-400" />
            <p className="text-sm text-slate-300">Loading graph view...</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && (
        <>
          <div className="absolute left-4 top-3 z-10 flex items-center gap-2">
            <div className="px-3 py-1.5 rounded-full bg-slate-900/90 border border-slate-700/50 text-xs text-slate-300 uppercase tracking-wider">
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at top, #020617 0%, #020417 55%)",
        }}
      />
    </div>
  );
